...
fn run() -> Result<()> {
    ...
    // Run in HFC mode, sync mode, or async mode (default)
    if config.hfc_mode {
        run_hfc(config)
    } else if config.use_sync {
        run_sync(config)
    } else {
...
fn run_sync(config: WalkConfig) -> Result<()> {
    ...
    // First Ctrl+C: graceful shutdown, second: immediate exit
...
async fn run_async_inner(config: WalkConfig) -> Result<()> {
    ...
    // Setup signal handler for graceful shutdown
    // First Ctrl+C: graceful shutdown with 10s watchdog, second: immediate exit
    let shutdown_flag = coordinator.shutdown_flag();
    ...
    ctrlc::set_handler(move || {
        ...
        if count == 0 {
            ...
            // Spawn watchdog thread to force exit if graceful shutdown takes too long
            std::thread::spawn(|| {
    ...
    let progress_task = if let Some(ref p) = progress {
        let stats = Arc::clone(coordinator.stats());
        let shutdown = Arc::clone(&shutdown_flag);
        let progress = Arc::clone(p);
        Some(tokio::spawn(async move {
            let start = std::time::Instant::now();
            while !shutdown.load(Ordering::Relaxed) {
                let dirs = stats.dirs_processed.load(Ordering::Relaxed);
                let files = stats.files_found.load(Ordering::Relaxed);
                let bytes = stats.bytes_found.load(Ordering::Relaxed);
                let queued = stats.entries_queued.load(Ordering::Relaxed);
                ...
                let msg = format!(
                    "Dirs: {} | Files: {} | Size: {} | Rate: {}/s | Queued: {} | Errors: {}",
                    dirs, files, size, rate, queued, errors
                );
